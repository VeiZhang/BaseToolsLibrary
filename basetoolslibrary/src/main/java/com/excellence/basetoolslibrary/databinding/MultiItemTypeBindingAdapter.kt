 ...
 *     blog   : http://tiimor.cn
 *     date   : 2017/10/16
 *     desc   : 开启dataBinding，ListView、GridView多布局通用适配器
 * </pre>
 */
open class MultiItemTypeBindingAdapter<T>() : BaseAdapter(), DataHelper<T> {
     ...
     * @return
     */
    fun addItemViewDelegate(delegate: ItemViewDelegate<T>): MultiItemTypeBindingAdapter<T> {
     ...
     * @return
     */
    fun addItemViewDelegate(viewType: Int, delegate: ItemViewDelegate<T>): MultiItemTypeBindingAdapter<T> {
     ...
     * @return
     */
    fun removeItemViewDelegate(delegate: ItemViewDelegate<T>): MultiItemTypeBindingAdapter<T> {
     ...
     * @return
     */
    fun removeItemViewDelegate(viewType: Int): MultiItemTypeBindingAdapter<T> {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View? {
        val delegate: ItemViewDelegate<T>? = getItemViewDelegate(getItemViewType(position))

        val binding: ViewDataBinding?
        binding = if (convertView == null) {
            DataBindingUtil.inflate(LayoutInflater.from(parent.context), delegate!!.getItemViewLayoutId(), parent, false)
        } else {
